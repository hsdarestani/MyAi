...
const NO_MODEL_FA='فعلاً مدل مناسب برای این کار از سمت Venice پیدا نشد. چند دقیقه بعد دوباره امتحان کن.';
...
export class VeniceModelRegistry{
  ...
  async select(task:TaskType,opts?:{model?:string;quality?:'fast'|'standard'|'high'}){
    ...
    if(!chosen) throw new Error(NO_MODEL_FA);
